    ...
    <>
      ...
      >
        <div className="flex items-center bg-black/20 rounded-2xl p-1 gap-1 border border-white/5 md:mr-4">
          {[
            { id: 'launcher', icon: isMobile && openWindows.length > 0 ? Home : LayoutGrid, onClick: toggleAppLauncher, label: 'App Launcher', active: isAppLauncherOpen, color: 'primary', shadow: '#cc97ff' },
            { id: 'about', icon: User, onClick: () => openWindow('about'), label: 'About Me', active: openWindows.includes('about'), color: 'secondary', shadow: '#00d2fd', hideMobile: true },
            { id: 'control', icon: isControlCenterOpen ? ChevronDown : ChevronUp, onClick: toggleControlCenter, label: 'Control Center', active: isControlCenterOpen, color: 'tertiary', shadow: '#00f5a0' }
          ].map(btn => (btn.hideMobile && isMobile) ? null : (
            <button
              key={btn.id}
              type="button"
              onClick={() => { btn.onClick(); playSound('click'); }}
              aria-label={btn.label}
              title={btn.label}
              className={`p-2.5 rounded-xl transition-all cursor-pointer group relative focus-visible:ring-2 focus-visible:ring-os-${btn.color}/50 focus-visible:bg-white/10 outline-none ${btn.active ? `bg-os-${btn.color}/10 border border-os-${btn.color}/30` : 'hover:bg-os-surfaceContainerLow/50 border border-transparent'}`}
            >
              <CustomIcon icon={btn.icon} size={20} color={btn.active ? `text-os-${btn.color}` : `text-os-onSurface group-hover:text-os-${btn.color}`} glow={btn.active ? `rgba(var(--os-${btn.color}-rgb), 0.5)` : false} />
              {btn.active && (
                <div
                  className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-os-${btn.color}`}
                  style={{ boxShadow: `0 0 8px ${btn.shadow}` }}
                />
              )}
            </button>
          ))}
            ...
            return (
              <button
                ...
                onClick={() => {
                  playSound('click');
                  if (isOpen) {
                    isActive ? toggleMinimizeWindow(app.id) : focusWindow(app.id);
                  } else openWindow(app.id);
                }}
                aria-label={app.title}
                title={app.title}
                className={`relative p-2.5 transition-all duration-300 cursor-pointer group flex items-center justify-center rounded-2xl focus-visible:ring-2 focus-visible:ring-os-primary/50 focus-visible:bg-white/10 outline-none ${isActive ? 'bg-white/10 shadow-[inset_0_0_12px_rgba(255,255,255,0.05)]' : 'hover:bg-os-surfaceContainerLow/50'}`}
              >
                ...
                {isOpen && (
                  <div className={`absolute -bottom-1 left-1/2 -translate-x-1/2 rounded-full transition-all duration-500 ${isActive ? 'w-2 h-1 animate-pulse' : 'w-1 h-1'}`}
                    style={{ backgroundColor: app.shadow || 'rgba(var(--os-primary-rgb), 1)', boxShadow: `0 0 10px ${app.shadow || 'rgba(var(--os-primary-rgb), 1)'}`, opacity: 0.8 }}
                  />
                )}
              </button>
            );
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {featuredApps.map((app) => (
                        <button
                          ...
                          title={app.title}
                          className="flex flex-col items-center justify-center p-2 rounded-2xl hover:bg-white/5 active:bg-white/10 transition-all cursor-pointer group focus-visible:ring-2 focus-visible:ring-os-primary/50 focus-visible:bg-white/10 outline-none"
                        >
                          <div className="p-3 bg-os-primary/10 rounded-2xl border border-os-primary/20 group-hover:bg-os-primary/20 group-hover:border-os-primary/40 transition-all mb-1 flex items-center justify-center">{app.icon(22)}</div>
                    ...
                    <div className={`grid grid-cols-4 gap-3 overflow-y-auto pr-1 flex-grow pb-2 scrollbar-hide`}>
                      {otherApps.map((app) => (
                        <button
                          ...
                          className="flex flex-col items-center justify-center p-2 rounded-2xl hover:bg-white/5 active:bg-white/10 transition-all cursor-pointer group focus-visible:ring-2 focus-visible:ring-os-primary/50 focus-visible:bg-white/10 outline-none"
                        >
                          <div className="p-2.5 bg-os-surfaceContainerLow/30 rounded-xl border border-white/5 group-hover:border-os-primary/30 transition-all mb-1">{app.icon(18)}</div>
                ...
                <div className="mt-auto bg-white/5 rounded-3xl p-4 border border-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="ml-auto flex items-center gap-1">
                      <button type="button" aria-label="Settings" title="Settings" className="p-2 hover:bg-white/10 rounded-xl transition-all cursor-pointer focus-visible:ring-2 focus-visible:ring-os-primary/50 focus-visible:bg-white/10 outline-none" onClick={() => { openWindow('settings'); toggleAppLauncher(); playSound('click'); }}><SettingsIcon size={16} className="text-os-onSurface" strokeWidth={2} /></button>
                      <button type="button" aria-label="Logout" title="Logout" className="p-2 hover:bg-red-500/20 rounded-xl transition-all cursor-pointer group/logout focus-visible:ring-2 focus-visible:ring-red-500/50 outline-none" onClick={() => { logout(); playSound('click'); }}><Power size={16} className="text-os-onSurfaceVariant group-hover/logout:text-red-500 transition-colors" /></button>
                    </div>
                  </div>
                </div>
              </div>
