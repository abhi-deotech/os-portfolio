...
const Taskbar = () => {
  ...
  return (
    <>
      ...
      >
        <nav className="flex items-center bg-black/20 rounded-2xl p-1 gap-1 border border-white/5 md:mr-4">
          <button
            type="button"
            onClick={toggleAppLauncher}
            aria-label={isAppLauncherOpen ? "Close Launcher" : "Open Launcher"}
            title={isAppLauncherOpen ? "Close Launcher" : "Open Launcher"}
          ...
          <button
            ...
            title={isControlCenterOpen ? "Close Control Center" : "Open Control Center"}
          ...
          {dockApps.map((app) => {
            ...
            return (
              <button
                key={app.id}
                type="button"
                aria-label={isOpen ? (isActive ? `Minimize ${app.title}` : `Focus ${app.title}`) : `Open ${app.title}`}
                title={isOpen ? (isActive ? `Minimize ${app.title}` : `Focus ${app.title}`) : `Open ${app.title}`}
                onClick={() => {
                  if (isOpen) {
                    if (isActive) {
