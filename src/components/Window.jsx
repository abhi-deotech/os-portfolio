...
const Window = ({ id, title, children, width = 900, height = 650, minWidth = 400, minHeight = 300 }) => {
  ...
  return (
    ...
    >
      ...
      <div 
        className="h-14 flex items-center px-6 border-b border-os-outline/5 relative shrink-0 transition-colors duration-300"
        onPointerDown={(e) => {
      ...
      >
        <div className="flex space-x-2.5 z-50 w-24 group/controls" onPointerDown={(e) => e.stopPropagation()}>
          <button 
            ...
            className="w-3.5 h-3.5 rounded-full bg-[#ff5f56] hover:brightness-110 flex items-center justify-center relative z-50 cursor-pointer border border-[#e0443e]"
          >
