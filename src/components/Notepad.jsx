...
const Notepad = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0c0c0c] text-sdl-ink font-sans selection:bg-os-primary/30">
      {/* Utility Bar */}
      <div className="flex items-center justify-between px-6 py-3 border-b border-hairline/5 bg-veil/[0.02]">
        ...
        <div className="flex items-center gap-2">
           <button 
             ...
             className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border border-hairline/5 text-[10px] font-bold uppercase tracking-widest transition-all ${isPreview ? 'bg-os-primary text-sdl-onAccent' : 'bg-veil/5 text-sdl-sec hover:text-sdl-onAccent'}`}
           >
           ...
           <button 
             onClick={handleSave}
             disabled={isSaved}
             className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${isSaved ? 'bg-veil/5 text-sdl-sec' : 'bg-green-500 text-sdl-onAccent hover:bg-green-400'}`}
           >
              <Save size={14} />
              Save
           </button>
      ...
      <div className="flex-grow relative overflow-hidden">
        {isPreview ? (
          ...
          <textarea
            ...
            spellCheck={false}
            className="absolute inset-0 w-full h-full bg-transparent p-10 outline-none resize-none font-mono text-sm leading-relaxed text-sdl-ink/80 placeholder:text-sdl-sec scrollbar-os"
      ...
      {/* Footer Info */}
      <div className="px-6 py-2 bg-black/40 border-t border-hairline/5 flex justify-between items-center">
         <div className="flex gap-4 text-[9px] font-bold text-sdl-sec uppercase tracking-widest">
