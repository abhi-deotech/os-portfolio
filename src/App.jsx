...
/**
 * Helper to render the application icon in the launcher with a consistent size.
 * Handles both direct CustomIcon components and wrapped icons (like Resume).
 * 
 * @param {Object} app - The app object from desktopIcons
 * @param {number} size - Optional size override for the icon
 * @returns {JSX.Element}
 */
const renderLauncherAppIcon = (app, size = 20) => {
  ...
  // Fallback: render the icon as is but attempt some scaling if possible
...
function App() {
  const {
    openWindows, activeWindow, openWindow, toggleControlCenter, isControlCenterOpen,
  ...
  return (
    ...
    >
      ...
      <div className={`flex-grow relative z-0 ${isMobile ? 'overflow-y-auto pt-8 pb-32 px-4' : 'overflow-hidden'}`}>
        ...
        {isMobile && (
          <div className="mt-12 mb-[calc(6rem+env(safe-area-inset-bottom))] px-4">
             <Widgets />
          </div>
        )}
      </div>
      ...
      >
        ...
        <div className={`flex items-center ${isMobile ? 'space-x-1 overflow-x-auto scrollbar-hide flex-grow' : 'space-x-1 flex-grow justify-center px-2'}`}>
          {(() => {
            ...
            // Filter out open windows that are NOT in the pinnedApps list
            const pinnedAppIds = pinnedApps.map(app => app.id);
            const unpinnedOpenApps = openWindows
              .filter(id => !pinnedAppIds.includes(id) && id !== 'about') // 'about' is on the left
              .map(id => {
                const appInfo = desktopIcons.find(icon => icon.id === id);
                if (!appInfo) return null;
                
                // Get icon component and color from the icon property
                let appIcon = null;
                ...
                if (appInfo.icon) {
                  if (appInfo.icon.type === CustomIcon) {
                    ...
                    // Handle wrapped icons (like CV/Resume)
            ...
            return allDockApps.map(({ id, icon: IconComponent, color, shadow, text }) => {
              ...
              <div className="flex flex-col space-y-4 h-full">
                ...
                <div className="space-y-3 flex-grow overflow-hidden flex flex-col">
                  ...
                  <div className="space-y-1.5 flex-grow overflow-hidden flex flex-col">
                    ...
                    <div className={`grid grid-cols-4 gap-3 overflow-y-auto pr-1 flex-grow pb-2`}>
                      {otherApps.map((app) => (
                        <div
                          key={app.id}
                          onClick={() => { openWindow(app.id); toggleAppLauncher(); }}
